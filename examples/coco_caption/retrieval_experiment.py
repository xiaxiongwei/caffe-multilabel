...
def main():
  ...
  if eval_on_test:
    ...
    ITER = 60000
    MODEL_FILENAME = 'lrcn_vgg16_iter_%d' % ITER
    #MODEL_FILENAME = 'lrcn_iter_%d' % ITER
    DATASET_NAME = 'val'
	#MODEL_FILENAME = 'lrcn_finetune_iter_%d' % ITER
  TAG += '_%s' % DATASET_NAME
  MODEL_DIR = '/mnt/lvmhdd/xiaxiongwei/models/coco_img_captioning'
  MODEL_FILE = '%s/%s.caffemodel' % (MODEL_DIR, MODEL_FILENAME)
  IMAGE_NET_FILE = './examples/coco_caption/vgg16.deploy.prototxt'
  ...
  for image_path, sentence in sg.image_sentence_pairs:
    if image_path not in dataset:
      dataset[image_path] = []
    dataset[image_path].append((sg.line_to_stream(sentence), sentence))   #dataset:{img_id:( [2,5,4,8], [i, am ,a ,man])}
